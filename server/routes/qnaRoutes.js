...
module.exports = app => {
  ...
  app.get('/api/questions/following/:hashtags', async (req, res) => {
    ...
    try {
      ...
      const data = await Question.paginate({ hashTag: { $elemMatch: { $in: regexpArray } } }, options);
  ...
  app.patch('/api/answers/:id', requireLogin, async (req, res) => {
    ...
    try {
      ...
      const targetQuestion = await Question.findById(answer.question);



      res.json(targetQuestion); // 수정 이후의 답변을 반환함.
  ...
  app.delete('/api/answers/:id', requireLogin, async (req, res) => {
    ...
    try {
      ...
      await User.findByIdAndUpdate(req.user._id, { $pull: { answeredQuestions: answer.question } }, { new: true });
